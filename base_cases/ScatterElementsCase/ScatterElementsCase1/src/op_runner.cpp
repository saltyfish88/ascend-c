...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnScatterElementsGetWorkspaceSize(inputTensor_[0],inputTensor_[1],inputTensor_[2],opDesc_->axis, opDesc_->reduce, 
    ...
    ret = aclnnScatterElements(workspace, workspaceSize, handle, stream);
    ...
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        if (aclrtMemcpy(hostInputs_[i], size, devInputs_[i], size, kind) != ACL_SUCCESS) {
