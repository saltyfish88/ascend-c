...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnDivGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnDiv(workspace, workspaceSize, handle, stream);
