...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnReplicationPad2dGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnReplicationPad2d(workspace, workspaceSize, handle, stream);
