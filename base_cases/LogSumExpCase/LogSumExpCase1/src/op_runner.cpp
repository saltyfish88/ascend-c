...
bool OpRunner::RunOp()
{
    ...
    if (opDesc_->dim.size() > 0)
    {
        dim = aclCreateIntArray(opDesc_->dim.data(), opDesc_->dim.size());
        if(dim==nullptr){
          printf("*****************\n");
        }
    }
    auto ret = aclnnLogSumExpGetWorkspaceSize(inputTensor_[0], dim, opDesc_->keepdim, outputTensor_[0],
	...
	INFO_LOG("Execute aclnnLogSumExpGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    if (aclnnLogSumExp(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnLogSumExp success");
