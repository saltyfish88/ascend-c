...
OperatorDesc CreateOpDesc()
{
    ...
    opDesc.rtol = 0.005;
    opDesc.atol = 0.005;
    opDesc.equal_nan = false;
    opDesc.AddInputTensorDesc(inputType, shape.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(inputType, shape.size(), shape.data(), format);

    opDesc.AddOutputTensorDesc(ACL_BOOL, shape.size(), shape.data(), format);
    return opDesc;
}
