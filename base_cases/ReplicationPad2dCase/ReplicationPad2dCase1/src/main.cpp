...
{
    // define operator
    std::vector<int64_t> shapex {4,16,16};
    std::vector<int64_t> shape_paddings {4};
    std::vector<int64_t> shape_y {4, 20,20};
    ...
    opDesc.AddInputTensorDesc(dataType2, shape_paddings.size(), shape_paddings.data(), format);
...
bool SetInputData(OpRunner &runner)
{
    ...
    ReadFile("../input/paddings.bin", fileSize, runner.GetInputBuffer<void>(1), runner.GetInputSize(1));
