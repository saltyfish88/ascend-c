...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnSoftmaxGetWorkspaceSize(inputTensor_[0],opDesc_->dim, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnSoftmax(workspace, workspaceSize, handle, stream);
